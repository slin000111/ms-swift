...
class Rollout(BaseUI):
    ...
    def do_build_ui(cls, base_tab: Type['BaseUI']):
        with gr.Row():
            gr.Slider(elem_id='temperature', minimum=0.0, maximum=10, step=0.1, value=0.3, scale=4)
            gr.Slider(elem_id='top_k', minimum=1, maximum=100, step=5, value=20, scale=4)
            gr.Slider(elem_id='top_p', minimum=0.0, maximum=1.0, step=0.05, value=0.7, scale=4)
            gr.Slider(elem_id='repetition_penalty', minimum=0.0, maximum=10, step=0.05, value=1.05, scale=4)
        with gr.Row():
            gr.Checkbox(elem_id='enable_prefix_caching', scale=4)
            ...
            gr.Slider(elem_id='gpu_memory_utilization', minimum=0.0, maximum=1.0, step=0.05, value=0.95, scale=4)
